    ...
    Private Sub subCheckStatus(ByVal Zone As String, ByVal Status() As String)
        ...
        Try
            ...
            If Zone = ZoneName Then
                ...
                End If
                'ASD 9/26/2016 added check, if you are both 0 then you can exit
                'If Status(0) = msLastStatus Then
                If (Status(0) = "0") And (msLastStatus(0) = "0") Then
                    'Nothing's new
                    Exit Sub
                Else
                    ...
                    If Status(0) = "0" Then
                        ...
                        'ASD 9/26/2016 changed the status return to a 1 array string for returnval
                        Dim returnval(0) As String

                        returnval(0) = msLastStatus
                        With goPLC
                            .ZoneName = goZone.Name
                            .TagName = AckTag
                            .PLCData = returnval  'Status
                        End With
                    ...
                    Else
                        ...
                        'ASD 9/26/2016 changes tp act status here..

                        'ASD 9/26/2016 changed the status return to a 1 array string for returnval
                        Dim returnval(0) As String
                        returnval(0) = Status(0)
...
#Region " Declares "
    ...
    Private mcolZones As clsZones = Nothing 'NRU 161006
    '******** End Module Variables ******************************************************************
    ...
    Private Function BuildAlarm(ByVal AlarmNumber As Integer, ByVal Active As Boolean) As clsPLCAlarm
        ...
        Try
            ...
            With oAlarm
                ...
                'NRU 161006 Determine zonenumber from the zonename that is already statically stored in this clsPLCAlarmDevice
                'moZone is dynamic and a liar.
                'BTK 01/16/08 Added code to log the zone number for plc generated zone alarms.
                '.Zone = moZone.ZoneNumber
                If mcolZones Is Nothing Then mcolZones = New clsZones(String.Empty)
                .Zone = mcolZones.ZoneNumberFromName(ZoneName)
    ...
    Private Sub subGetAlarms(ByVal Index As Integer)
        ...
        If Index = BitIndex Then
            ...
            Finally
                ...
                'ASD 9/26/2016 dont do this here
