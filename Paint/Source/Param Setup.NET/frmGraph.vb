...
#Region " Declares "
    ...
    Private mnAxisYMin(1) As Integer 'Max scale on each side
    Private mnAxisYRange(1) As Integer 'Max scale on each side
...
#Region " Properties "
    Friend Property NumLines() As Integer
        ...
        Set(ByVal value As Integer)
            ...
            For nIndex As Integer = 0 To mnNumLines - 1
                ...
                mLineCfgs(nIndex).MinScale = 0
                mLineCfgs(nIndex).MaxScale = 100
                mLineCfgs(nIndex).Range = mLineCfgs(nIndex).MaxScale - mLineCfgs(nIndex).MinScale
    ...
    Friend Property MaxScale(ByVal nIndex As Integer) As Integer
        ...
        End Set
    End Property
    Friend Property MinScale(ByVal nIndex As Integer) As Integer
        '********************************************************************************************
        'Description: Get or set the min value for this line.
        '           
        'Parameters: none
        'Returns:    none
        '
        ...
        '********************************************************************************************
        Get
            Return mLineCfgs(nIndex).MinScale
        End Get
        Set(ByVal value As Integer)
            mLineCfgs(nIndex).MinScale = value
            mLineCfgs(nIndex).Range = mLineCfgs(nIndex).MaxScale - mLineCfgs(nIndex).MinScale
        End Set
    End Property
#End Region
    ...
    Private Sub subInitFormText()
        ...
        For nLine As Integer = 0 To mnNumLines - 1
            ...
            If mLineCfgs(nLine).YAxis = 1 Then
                mbUseYAxis1 = True
            End If
            If mLineCfgs(nLine).MinScale < mnAxisYMin(mLineCfgs(nLine).YAxis) Then
                mnAxisYMin(mLineCfgs(nLine).YAxis) = mLineCfgs(nLine).MinScale
            End If
            ...
            End If
            mnAxisYRange(mLineCfgs(nLine).YAxis) = mnAxisYMax(mLineCfgs(nLine).YAxis) - mnAxisYMin(mLineCfgs(nLine).YAxis)
        ...
        End If
        'YAxis scales
        lblYAxis0Bottom.Text = mnAxisYMin(0).ToString
        lblYAxis1Bottom.Visible = mbUseYAxis1
        lblYAxis1Top.Visible = mbUseYAxis1
        lblYAxis1Bottom.Text = mnAxisYMin(1).ToString
...
#End Region
    ...
    Private Sub pnlGraph_Paint(ByVal sender As Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles pnlGraph.Paint
        ...
        For nline As Integer = 0 To nLines
            fVertScale(nline) = (pnlGraph.Height / mnAxisYRange(mLineCfgs(nline).YAxis)) / 10
            nX0(nline) = 0
            nY0(nline) = pnlGraph.Height - CInt((mnLineData(nline, nArrayIndex) - (mnAxisYMin(mLineCfgs(nline).YAxis) * 10)) * fVertScale(nline))
            nX1(nline) = 0
            nY1(nline) = 0
            nIndex2 = nArrayIndex
            For nIndex As Integer = nStart To nStop
                ...
                nY1(nline) = pnlGraph.Height - CInt((mnLineData(nline, nIndex2) - (mnAxisYMin(mLineCfgs(nline).YAxis) * 10)) * fVertScale(nline))
