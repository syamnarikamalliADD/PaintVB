...
'    01/06/2014 MSW     Make ZoneName available to make it easier to switch with the dll version 4.01.06.00
'********************************************************************************************
...
#Region "  Declares  "
    ...
    Private msTagParam1 As String = String.Empty
    Private msTagParam2 As String = String.Empty
...
#Region " Properties "
    ...
    End Property
    Public Property TagParam1() As String
        '********************************************************************************************
        'Description:  TagPram1 - Value to replace "#" with in the tag
        '
        'Parameters: none
        'Returns:    none
        '
    ...
    Public Property TagParam2() As String
        '********************************************************************************************
        'Description:  TagPram1 - Value to replace "%" with in the tag
        '
        'Parameters: none
    ...
    Friend Property ZoneName() As String
        ...
        End Get
        Set(ByVal value As String)
            If GetDefaultFilePath(msXMLFilePath, mPWCommon.eDir.XML, "", msXMLFILENAME) Then
                msZoneName = value
            Else
                msZoneName = String.Empty
                Dim sTmp As String = gcsRM.GetString("csCANT_FIND_CONFIG_FILE") & _
...
#Region " Routines "
    ...
    Private Sub subCreateHotLink(ByRef Link As clsLink)
        ...
        Try
            ...
            oControl.Name = "AxAbtcp" & mnHotLinks.ToString(CultureInfo.InvariantCulture)
            oControl.BeginInit()
                    ...
                    If (oAsabtcp.Name <> "AxAsabtcp0") Or mbRead Then
                        ...
                        For nIndex As Short = 0 To nUBound 'CType(.Length - 1, Short)
                            Select Case .Type
                                Case "SINT"
                                    sReadData(nIndex) = _
                                        oAsabtcp.GetDataByteM(nIndex).ToString(CultureInfo.InvariantCulture)
                                Case "INT"
                                    sReadData(nIndex) = _
                                        oAsabtcp.GetDataWordM(nIndex).ToString(CultureInfo.InvariantCulture)
                                Case "DINT"
                                    sReadData(nIndex) = _
                                        oAsabtcp.GetDataLongM(nIndex).ToString(CultureInfo.InvariantCulture)
                                Case "REAL"
                                    sReadData(nIndex) = _
                                        oAsabtcp.GetDataFloatM(nIndex).ToString(CultureInfo.InvariantCulture)
                                Case "BIT16", "BIT32", "BOOL"
            ...
            With oAsabtcp
                ...
                For nIndex As Short = 0 To CType(.SlaveMemQty - 1, Short)
                    Select Case oLink.Type
                        Case "SINT"
                            sReadData(nIndex) = _
                                .get_SlaveDataWord(nIndex).ToString(CultureInfo.InvariantCulture)
                        ...
                        Case "DINT"
                            sReadData(nIndex) = _
                                .get_SlaveDataLong(nIndex).ToString(CultureInfo.InvariantCulture)
                        Case "REAL"
                            sReadData(nIndex) = _
                                .get_SlaveDataFloat(nIndex).ToString(CultureInfo.InvariantCulture)
