...
'    12/11/14   MSW     Semiauto charts for Ford                                        4.01.07.05
'                       Working on the daily checkbox.  It's disabled by default        
'                       frmCriteria const mbShowDaily can enable it.  Still needs work
'                       for downtime reports
'    01/30/15   RJO     Populate Device list with all devices for Alarm Reports.        4.01.07.06
'******************************************************************************************************
Option Compare Text
...
#Region " Routines "
                ...
                Case Keys.F1
                    ...
                    Select Case ReportType
                        ...
                        Case eReportType.Downtime
                            subLaunchHelp(gs_HELP_REPORTS_DOWNTIME, oIPC)
                        Case eReportType.RMCharts, eReportType.RMChartsAuto, eReportType.RMChartsSemiAuto
                ...
                Case Keys.F2
                    ...
                    Select Case ReportType
                        ...
                        Case eReportType.RMChartsAuto
                            sName = sName & "_Auto"
                        Case eReportType.RMChartsSemiAuto
                            sName = sName & "SemiAuto"
                        Case eReportType.Vision
                            sName = sName & "_Vision"
                    End Select
    ...
    Friend Function dtGetDowntimeData(ByVal TableName As String, _
        ...
        Dim dtStartTimes(0) As DateTime
        Dim dtEndTimes(0) As DateTime

        Dim bDaily As Boolean = frmCriteria.chkDaily.Checked
        Try

            If bDaily Then
                dtStartTimes(0) = frmCriteria.StartTime
                Dim dtEndTimeTmp As DateTime = frmCriteria.EndTime
                Dim nCriteriaStart As Integer = dtStartTimes(0).Hour * 10000 + dtStartTimes(0).Minute * 100 + dtStartTimes(0).Second
                Dim nCriteriaEnd As Integer = dtEndTimeTmp.Hour * 10000 + dtEndTimeTmp.Minute * 100 + dtEndTimeTmp.Second
                Dim bOvernight As Boolean = False
                Dim nDays As Integer = dtEndTimeTmp.Day - dtStartTimes(0).Day
                If nCriteriaEnd < nCriteriaStart Then
                    bOvernight = True
                    nDays = nDays - 1
                Else
                    bOvernight = False
                End If
                ReDim dtStartTimes(nDays - 1)
                ReDim dtEndTimes(nDays - 1)
                dtEndTimes(nDays) = dtEndTimeTmp
                For nDay As Integer = 1 To nDays - 1
                    dtStartTimes(nDay) = DateAdd(DateInterval.Day, nDay, dtStartTimes(0))
                    dtEndTimes(nDay - 1) = DateAdd(DateInterval.Day, ((nDay - 1) - nDays), dtStartTimes(0))
                Next
            End If
            ...
            dtNewTable = dtConveyor.Clone
            Dim dLog1 As Date = Now


            Debug.Print("dtGetDowntimeData Start loop - " & DateDiff(DateInterval.Second, dLog1, Now).ToString )
            
            ' alarm table is latest first
            nDTPtr = dtDown.Rows.Count - 1
            Dim bFirst As Boolean = True
            'Setup some local variables to speed things up
            Dim dCrtEndTime As Date = frmCriteria.EndTime
            Dim sALARM_CAP As String = gpsRM.GetString("psALARM_CAP")
            Dim sSTART_TIME_CAP As String = gpsRM.GetString("psSTART_TIME_CAP")
            Dim sEND_TIME_CAP As String = gpsRM.GetString("psEND_TIME_CAP")
            Dim sDESC_CAP As String = gpsRM.GetString("psDESC_CAP")
            Dim sVIN_CAP As String = gpsRM.GetString("psVIN_CAP")
            Dim sCARRIER_CAP As String = gpsRM.GetString("psCARRIER_CAP")
            Dim sSTYLE_CAP As String = gpsRM.GetString("psSTYLE_CAP")
            Dim sCOLOR_CAP As String = gpsRM.GetString("psCOLOR_CAP")
            Dim sUNKNOWN As String = gpsRM.GetString("psUNKNOWN")
            Dim bVin As Boolean = frmCriteria.bShowColumn(frmCriteria.eShowCol.VIN)
            Dim bCarrier As Boolean = frmCriteria.bShowColumn(frmCriteria.eShowCol.Carrier)
            Dim bColor As Boolean = frmCriteria.bShowColumn(frmCriteria.eShowCol.Color)
            Dim nCnvAlmCol As Integer = -1
            Dim nCnvStrtCol As Integer = -1
            Dim nCnvEndCol As Integer = -1
            Dim nCnvDescCol As Integer = -1
            Dim nCnvVinCol As Integer = -1
            Dim nCnvCarrierCol As Integer = -1
            Dim nCnvStyleCol As Integer = -1
            Dim nCnvColorCol As Integer = -1
            For nCol As Integer = 0 To dtConveyor.Columns.Count - 1
                Select Case dtConveyor.Columns.Item(nCol).ColumnName
                    Case sALARM_CAP
                        nCnvAlmCol = nCol
                    Case sSTART_TIME_CAP
                        nCnvStrtCol = nCol
                    Case sEND_TIME_CAP
                        nCnvEndCol = nCol
                    Case sDESC_CAP
                ...
                End Select
            Next
            Dim nDTAlmCol As Integer = -1
            Dim nDTStrtCol As Integer = -1
            Dim nDTEndCol As Integer = -1
            Dim nDTDescCol As Integer = -1
            Dim nDTVinCol As Integer = -1
            Dim nDTCarrierCol As Integer = -1
            Dim nDTStyleCol As Integer = -1
            Dim nDTColorCol As Integer = -1
            For nCol As Integer = 0 To dtDown.Columns.Count - 1
                Select Case dtDown.Columns.Item(nCol).ColumnName
                    Case sALARM_CAP
                        nDTAlmCol = nCol
                    Case sSTART_TIME_CAP
                        nDTStrtCol = nCol
            ...
            For Each dr As DataRow In dtConveyor.Rows
                ...
                Dim sAlarmCode As String = Trim(dr.Item(nCnvAlmCol).ToString)
                dtStart = CType(dr.Item(nCnvStrtCol), DateTime)
                Select Case sAlarmCode
                    Case gtConvAlarms.HeldByFanuc, gtConvAlarms.HeldByFanucOS
                        ...
                        If (sAlarmCode = gtConvAlarms.HeldByFanuc) Or bFirst Or _
                           Math.Abs(DateDiff(DateInterval.Second, dtStart, dtEnd)) > 2 Then
                            bFirst = False
                            'start of fanuc downtime
                            'dtStart = CType(dr.Item(gpsRM.GetString("psSTART_TIME_CAP")), DateTime)
                            dtEnd = CType(dr.Item(nCnvEndCol), DateTime)
                            If dtEnd > dCrtEndTime Then
                                dtEnd = dCrtEndTime
                            End If
                            If bDaily Then
                                Dim nAlarmEnd As Integer = dtEnd.Hour * 10000 + dtEnd.Minute * 100 + dtEnd.Second
                                Dim nAlarmStart As Integer = dtStart.Hour * 10000 + dtStart.Minute * 100 + dtStart.Second
                                Dim nCriteriaEnd As Integer = frmCriteria.EndTime.Hour * 10000 + frmCriteria.EndTime.Minute * 100 + frmCriteria.EndTime.Second
                                If nAlarmStart < nCriteriaEnd And nAlarmEnd > nCriteriaEnd Then
                                    dtEnd = New Date(dtEnd.Year, dtEnd.Month, dtEnd.Day, frmCriteria.EndTime.Hour, frmCriteria.EndTime.Minute, frmCriteria.EndTime.Second)
                                End If
                            End If
                            nDTPtr = dtDown.Rows.Count - 1
                            ...
                            Do While True
                                ...
                                dtAlmStart = CType(drDT.Item(nDTStrtCol), DateTime)
                                ...
                                If Math.Abs(DateDiff(DateInterval.Second, dtStart, dtAlmStart)) <= 5 Then
                                    ...
                                    Dim sAlarm As String = drDT.Item(nDTAlmCol).ToString
                                    If DTAlarmNotMasked(sAlarm) Or (bFoundABetterAlarm = False) Then  'MSW 4/8/14
                                        ...
                                        If (bFoundABetterAlarm = False) Or (InStr(sAlarm, "PLCZ") > 0) Then
                                            dr.Item(nCnvAlmCol) = sAlarm
                                            dr.Item(nCnvDescCol) = drDT.Item(nDTDescCol)
                                            ...
                                            If bVin Then
                                                dr.Item(nCnvVinCol) = drDT.Item(nDTVinCol)
                                            End If

                                            If bCarrier Then
                                                dr.Item(nCnvCarrierCol) = drDT.Item(nDTCarrierCol)
                                            End If

                                            dr.Item(nCnvStyleCol) = drDT.Item(nDTStyleCol)

                                            If bColor Then
                                                dr.Item(nCnvColorCol) = drDT.Item(nDTColorCol)
                                            End If
            ...
            Debug.Print("dtGetDowntimeData After loop- " & DateDiff(DateInterval.Second, dLog1, Now).ToString)
            dLog1 = Now
        ...
        Select Case ReportType
            ...
            Case eReportType.Conveyor
                ...
                If frmCriteria.chkSummary.Checked Then
                    ...
                    ChartConfig.Daily = frmCriteria.chkDaily.Checked
            ...
            Case eReportType.Downtime, eReportType.RMCharts, eReportType.RMChartsAuto, eReportType.RMChartsSemiAuto
    ...
    Friend Sub subDoChart(ByRef oPicChart As PictureBox, ByRef oPicKey As PictureBox, ByRef oDT As DataTable, _
        ...
        Select Case ReportType
            ...
            Case eReportType.RMCharts
                sPieTitle = gpsRM.GetString("psPIE_TITLE_DT") & sSummType
                nItems = 10
            Case eReportType.RMChartsAuto, eReportType.RMChartsSemiAuto
                sPieTitle = sTitle
                nItems = 5
            Case Else
        ...
        Select Case oChartType
            Case eChartTypes.Bar
                ...
                With oBar
                    ...
                    If (ReportType = eReportType.RMChartsAuto) Or (ReportType = eReportType.RMChartsSemiAuto) Then
                        .KeyFontSize = 10
                        .KeyTitleFontSize = 12
                    Else
                        .KeyTitleFontSize = 15
    ...
    Private Sub subInitializeForm()
        ...
        Try
            ...
            If ReportType = eReportType.Unknown Then
                ...
                btnSelect.DropDownItems.Add(gpsRM.GetString("psMANUAL_WEEKLY_REPORT"))
            ...
            Do While Not mbGotScreenList
                Application.DoEvents()
                If DateDiff(DateInterval.Second, oDate, Now) > 10 Then
            ...
            If (ReportType <> eReportType.Unknown) Then
                ...
                Select Case ReportType
                    ...
                    Case eReportType.RMChartsSemiAuto
                        lstStatus.Visible = True
                        frmSemiAutoDates.ZoneCollection = colZones
                        frmSemiAutoDates.ShowMe(mdCmdDate)
                        mbShutdown = True
                        Me.Close()
                        Exit Sub
                    Case Else
            ...
            'Dim oController As clsController = Nothing
            msFanucName = String.Empty
            For Each oController As clsController In colControllers
                If oController.RCMConnectStatus = FRRobotNeighborhood.FRERNConnectionStatusConstants.frRNConnected Then
        ...
        Catch ex As Exception
            ...
            End Select
        Finally
            If (ReportType <> eReportType.RMChartsAuto) And (ReportType <> eReportType.RMChartsSemiAuto) And Me.Visible Then
        ...
        For Each sTmp As String In My.Application.CommandLineArgs
            Select Case Strings.LCase(sTmp)
                ...
                Case "weekly", "auto"
                    ...
                    btnCriteria.Enabled = False
                Case "manualweekly", "semiauto"
                    ReportType = eReportType.RMChartsSemiAuto
                    Me.Text = gpsRM.GetString("psMANUAL_WEEKLY_REPORT")
                    btnCriteria.Enabled = False
                Case "vision"
    ...
    Private Sub btnSelect_DropDownItemClicked(ByVal sender As Object, _
        ...
        Select Case e.ClickedItem.Text
            ...
            Case gpsRM.GetString("psWEEKLY_REPORT")
                ReportType = eReportType.RMChartsAuto
                btnCriteria.Enabled = False
            Case gpsRM.GetString("psMANUAL_WEEKLY_REPORT")
                ReportType = eReportType.RMChartsSemiAuto
                btnCriteria.Enabled = False
                    ...
                    Try
                        ...
                        If Not (sAlarmCode Is Nothing) Then
                            ...
                            Select Case sHelpFileName.ToUpper 'RJO 03/12/13
                                ...
                                Case Else
                                    If msFanucName = String.Empty Then
                                        'This is a PLC or GUI generated alarm. Cause/Remedy is in the Help\AlarmMan folder.
                                        hrefAddress = msHelpPath & "AlarmMan/" & sHelpFileName & ".HTM#" & sBookMark
                                    Else
