...
#Region " Declares "
    ...
    Private Const mbShowDaily As Boolean = False 'NRU 160929 This didn't match the stored procedure in database true to false.
...
#Region " Routines "
            ...
            Select Case ReportType
                ...
                Case frmMain.eReportType.Downtime  '----------------------------------------------------
                    ...
                    Else
                        ...
                        subBuildDownTimeProcedure(s)
                    End If
                Case frmMain.eReportType.RMCharts, frmMain.eReportType.RMChartsAuto, frmMain.eReportType.RMChartsSemiAuto '----------------------------------------------------
                ...
                Case frmMain.eReportType.Conveyor
                    ...
                    If mbGetLastConvOSAlarm Then
                        s = DB.GetStoredProcedureCommand("GetTopxxRecords")
                        subBuildConveyorProcedure(s, True)
                    Else
                        s = DB.GetStoredProcedureCommand("GetAlarmRecords")
                        subBuildConveyorProcedure(s, False)
                    End If
    ...
    Private Function oGetDeviceBox(ByVal bAddControllers As Boolean, Optional ByVal bAddArms As Boolean = True) As CheckedListBox
        ...
        ' 01/30/15  RJO     Populate Device list with all devices for Alarm Reports.
        '********************************************************************************************
        Try
            ...
            '01/30/15 RJO Use PLC Alarm Devices.XML list to build Device list for Alarm Reports
            If mReportType = (frmMain.eReportType.Alarm) And (Not colZones.StandAlone) Or mReportType = (frmMain.eReportType.Change) And (Not colZones.StandAlone) Then
                ...
                Try
                    If GetDefaultFilePath(sXMLFilePath, eDir.XML, String.Empty, sXMLFILE & ".XML") Then
                        Try
                            oXMLDoc.Load(sXMLFilePath)
                            For Each oZone As clsZone In colZones   'RV 10/7/16
                                oMainNode = oXMLDoc.SelectSingleNode("//" & sXMLTABLE)
                                oNodeList = oMainNode.SelectNodes("//" & oZone.Name & "//" & sXMLNODE)
                                If oNodeList.Count > 0 Then
                                    For Each oNode As XmlNode In oNodeList
                                        o.Items.Add(oNode.Item("DeviceName").InnerText)
                                    Next
                                End If
                        ...
                        Catch ex As Exception
                            mDebug.WriteEventToLog(frmMain.msSCREEN_NAME & " Module: frmCriteia Routine: oGetDeviceBox", _
                                                   "Error: " & ex.Message & vbCrLf & "StackTrace: " & ex.StackTrace)
                        End Try
                    End If
                Catch ex As Exception
                    mDebug.WriteEventToLog("Module: frmCriteia Routine: oGetDeviceBox", _
                                           "Error: " & ex.Message & vbCrLf & "StackTrace: " & ex.StackTrace)
                End Try
            Else
                'For Each oZone As clsZone In colZones   'RV 10/7/16
    ...
    End Sub
    Private Sub subBuildConveyorProcedure(ByRef s As SqlClient.SqlCommand, ByVal bTOP As Boolean)
        '********************************************************************************************
        ...
        With s.Parameters
            ...
            If mbShowDaily And (bTOP = False) Then
                If chkDaily.Enabled And chkDaily.Checked Then
                    .Add(New SqlClient.SqlParameter("@Daily", _
                        SqlDbType.Bit)).Value = 1
                Else
    ...
    Private Sub subBuildVisionFieldParams(ByRef s As SqlClient.SqlCommand)
        ...
        With s.Parameters
            ...
            ' Vision Status X
            .Add(New SqlClient.SqlParameter("@Vision_Status_X_Cap", _
                SqlDbType.NVarChar)).Value = gpsRM.GetString("psVISION_STATUS_X_CAP")

            ' Vision Status Y
            .Add(New SqlClient.SqlParameter("@Vision_Status_Y_Cap", _
                SqlDbType.NVarChar)).Value = gpsRM.GetString("psVISION_STATUS_Y_CAP")

            ' Vision Status Z
            .Add(New SqlClient.SqlParameter("@Vision_Status_Z_Cap", _
                SqlDbType.NVarChar)).Value = gpsRM.GetString("psVISION_STATUS_Z_CAP")


            ' Vision Status W
            .Add(New SqlClient.SqlParameter("@Vision_Status_W_Cap", _
                SqlDbType.NVarChar)).Value = gpsRM.GetString("psVISION_STATUS_W_CAP")

            ' Vision Status P
            .Add(New SqlClient.SqlParameter("@Vision_Status_P_Cap", _
                SqlDbType.NVarChar)).Value = gpsRM.GetString("psVISION_STATUS_P_CAP")

            ' Vision Status R
            .Add(New SqlClient.SqlParameter("@Vision_Status_R_Cap", _
                SqlDbType.NVarChar)).Value = gpsRM.GetString("psVISION_STATUS_R_CAP")
    ...
    Private Sub subSetZoneForRobotCollections(ByVal Zonename As String)
        ...
        If ReportType = frmMain.eReportType.Production Then 'NVSP production reports change. Device box needs to get loaded with all the devices
            colControllers = New clsControllers(colZones, colZones.StandAlone, True)
        Else
    ...
    Private Sub subInitializeForm()
        ...
        Try
            ...
            End If
            'Set some locations for easier design time - 

            gpbShift.Location = New Point(12, 80)
            gpbShift.Size = New Size(446, 86)

            gpbOptions.Location = New Point(484, 80)
            gpbOptions.Size = New Size(392, 86)

            gpbSummaryOpt.Location = New Point(12, 175)
            gpbSummaryOpt.Size = New Size(864, 50)

            tlpMain.Location = New Point(16, 184)
            tlpMain.Size = New Size(839, 317)
    ...
    End Sub
    Friend Sub subSetupSemiAutoCriteria()
        '********************************************************************************************
        'Description:  Set me up some more
        '
        'Parameters: 
    ...
    Private Sub subSetupProductionCriteria()
        ...
        Dim lb1 As CheckedListBox = oGetDeviceBox(False, True) 'NVSP 12/16/2016 - Either RC or Arm should be displayed because 1 controller is 1 arm here
        AddHandler lb1.ItemCheck, AddressOf subCLBCheckHandler
        tlpMain.Controls.Add(lb1, 1, 1)
...
#End Region
    ...
    Private Sub frmCriteria_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        '********************************************************************************************
        'Description:  Run automatic reports, position pieces on screen for different reports
        '
        'Parameters: 
        'Returns:    
        '
