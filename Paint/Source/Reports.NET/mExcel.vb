...
'    08/05/14   MSW     Merge Reports and Reports RM folder, rename this module mCharts, so it can share 
'                       the same frmMain.
'******************************************************************************************************
'
'******************************************************************************************************
...
'EXCEL12 = False for Excel 2003 (v11)
'******************************************************************************************************
#Const EXCEL12 = False
'******************************************************************************************************
        ...
        Try
            ...
            If DT.Columns.Count > 0 Then
                ...
                For Each dr As DataRow In DT.Rows
                    ...
                    StartDate = CType(dr.Item(gpsRM.GetString("psSTART_TIME_CAP")), Date).Date
                    For nCol As Integer = ChartConfig.TableCol To DT.Columns.Count + ChartConfig.TableCol - 1
    ...
    Friend Function subDoManualChart(ByVal TableIn As DataTable, _
        ...
        Try
            ...
            With o
                ...
                .Filter = "All Files|*.*|Excel File|*." & sXLS_EXT
                .DefaultExt = "." & sXLS_EXT
                .FilterIndex = 3
...
#End If
    ...
    Private Sub subKillExcels()
        ...
        Thread.Sleep(100)
        Application.DoEvents()
        Dim oProcs() As Process = Process.GetProcessesByName("EXCEL")
    ...
    Private Sub subMoveChart(ByRef oXl As Excel.Application, _
        ...
        If FullPage Then
            ...
            Dim oChartObject As Excel.ChartObject = DirectCast(oChartObjects.Item(oChartObjects.Count), Excel.ChartObject)
            oChartObject.Chart.ChartTitle.Font.Size = 14
        Else
...
#End If
            ...
            Dim oChartObject As Excel.ChartObject = DirectCast(oChartObjects.Item(oChartObjects.Count), Excel.ChartObject)
            oChartObject.Chart.ChartTitle.Font.Size = 12
        End If
    ...
    End Sub
    Private Sub subAbortAutoChart(ByVal sTarget As String, ByVal oXl As Excel.Application, _
                                  ByVal oTableSheet As Excel.Worksheet, ByVal oChartSheet As Excel.Worksheet, _
                                  ByVal oYearlySheet As Excel.Worksheet, ByVal oYearlyStatsSheet As Excel.Worksheet, _
                                  ByVal oWb As Excel.Workbook, ByVal oWbYearly As Excel.Workbook)
        '********************************************************************************************
        'Description:
        'Parameters:
        'Returns:
    ...
    Friend Sub subDoAutoCharts(Optional ByRef dDate As Date = Nothing, _
        ...
        Try
            ...
            If (dDate = Nothing) Then
                ...
                dPreviousDate = DateAdd(DateInterval.WeekOfYear, -1, dEndTime)
                dPreviousBeginDate = DateAdd(DateInterval.WeekOfYear, -1, dStartTime)
            Else
                dEndTime = dDate
                If dBeginDate = Nothing Then
                ...
                Else
                    'Manual range
                    dStartTime = dBeginDate
                    If dPreviousDate = Nothing Then
                        Dim nDays As Long = DateDiff(DateInterval.Day, dEndTime, dStartTime) - 1
                        dPreviousDate = DateAdd(DateInterval.Day, nDays, dEndTime)
                        dPreviousBeginDate = DateAdd(DateInterval.Day, nDays, dStartTime)
                    ElseIf dPreviousBeginDate = Nothing Then
                        Dim nDays As Long = DateDiff(DateInterval.Day, dEndTime, dPreviousDate)
                        dPreviousBeginDate = DateAdd(DateInterval.Day, nDays, dStartTime)
                    End If
                End If
            End If
            If bAuto Then
                dStart2MonthTime = DateAdd(DateInterval.Month, -2, dEndTime)
                dStart2MonthTime = DateAdd(DateInterval.Second, 1, dStart2MonthTime)
            Else
                dStart2MonthTime = dPreviousBeginDate
            End If
            frmMain.Status = String.Format(gpsRM.GetString("psCREATING_AUTO_CHART"), dEndTime.ToString)
        ...
        Try
            ...
            Dim sFileName As String = frmMain.colZones.CurrentZone & " - "
            If bAuto Then
                sFileName = sFileName & gpsRM.GetString("psWEEKLY_REPORT_FILENAME") & " - " & _
                                           dEndTime.Date.ToString("MMMM-dd-yyyy")
            Else
                sFileName = sFileName & "Semi-" & gpsRM.GetString("psWEEKLY_REPORT_FILENAME") & " - " & _
                                           dEndTime.Date.ToString("MMMM-dd-yyyy")
                Dim o As New SaveFileDialog
                With o
                    .Filter = gcsRM.GetString("csSAVE_CSV_CDFILTER")
            ...
            '*******************************************************************************
            'Two Month Chart
            Call OccurrenceDuration(oXl, oWb, oChartSheet, ChartConfig, bContinue, dEndTime, dStart2MonthTime)

            If bContinue = False Then
                'No Data
                subAbortAutoChart(sTarget, oXl, oTableSheet, oChartSheet, oYearlySheet, oYearlyStatsSheet, oWb, oWbYearly)
            ...
            For nWeek As Integer = nTHIS_WEEK To nLAST_WEEK
                Select Case nWeek
                    ...
                    Case nLAST_WEEK 'Last week, adjust the dates
                        ...
                        ChartConfig.StartTime = dPreviousBeginDate
                        ChartConfig.EndTime = dPreviousDate
                        nCol = 10
                End Select
                frmMain.Status = String.Format(gpsRM.GetString("psGEN_CHARTS_FOR"), ChartConfig.StartTime.ToString, ChartConfig.EndTime.ToString)
                ...
                If DT1.Columns.Count = 0 Or DT1.Rows.Count = 0 Then
                    'No Data
                    subAbortAutoChart(sTarget, oXl, oTableSheet, oChartSheet, oYearlySheet, oYearlyStatsSheet, oWb, oWbYearly)
                ...
                If nWeek = nTHIS_WEEK And bAuto Then
                    '********************************************************************************************
                ...
                For nTable As Integer = nBY_DUR To nBY_OCCUR
                    ...
                    ChartConfig.TopxxReq = 5
                    subGetChartData(DT1, DTsumm, sTitle, ChartConfig)
                    If DTsumm.Columns.Count = 0 Or DT1.Rows.Count = 0 Then
                        'No Data
                        subAbortAutoChart(sTarget, oXl, oTableSheet, oChartSheet, oYearlySheet, oYearlyStatsSheet, oWb, oWbYearly)
            ...
            If bAuto Then
                ...
                oXl.Visible = mPWCommon.InIDE Or (bAuto = False)
...
#End If
            ...
            If Not bAuto Then 'And (Not InIDE) Then
                Application.DoEvents()
                Thread.Sleep(100)
                Application.DoEvents()
                Thread.Sleep(100)
    ...
    Private Sub OccurrenceDuration(ByRef oXl As Excel.Application, _
                                ...
                                Optional ByRef dDate As Date = Nothing, _
                                Optional ByRef dStartDate As Date = Nothing)
        '********************************************************************************************
        ...
        If dStartDate = Nothing Then
            ...
            dStartTime = DateAdd(DateInterval.Second, 1, dStartTime)
        Else
            dStartTime = New Date(dStartDate.Year, dStartDate.Month, dStartDate.Day, 0, 0, 0)
        End If
        ...
        'setup chart config for the location of the first chart
        '********************************************************************************************
        ChartConfig.ChartSheet = gpsRM.GetString("psALARMS_SHEET_NAME") ' gpsRM.GetString("psPAST_MON") & gpsRM.GetString("psALARMS_SHEET_NAME")
        ...
        If lLastRow < 4 Then
            lLastRow = 4
        End If
        sSourceString = gpsRM.GetString("psALARMS_SHEET_NAME") & "!R3C1:R" & lLastRow & "C" & nLastCol
        'Debug.Print(sSourceString)
        sDestString = gpsRM.GetString("psALARMS_SHEET_NAME") & "!R1C" & (nLastCol + 9)   '"Sheet1!R1C" & (nLastCol + 4)
        ...
        PvtTable = DirectCast(oChartSheet.PivotTables(sPivotTable), Excel.PivotTable)
        ...
        PvtTable = DirectCast(oChartSheet.PivotTables(sPivotTable), Excel.PivotTable)
        pvtField = DirectCast(PvtTable.PivotFields("Duration"), Excel.PivotField)
        With pvtField
        ...
        oChart = DirectCast(oXl.Charts.Add, Excel.Chart)
        With oChart
        ...
        With oXl.ActiveChart
            xlSeries = DirectCast(.SeriesCollection, Excel.SeriesCollection)
            ...
            .PlotArea.Select()
            .HasTitle = True
            xlAxisCategory = DirectCast(.Axes(, Excel.XlAxisGroup.xlPrimary), Excel.Axes)
            ...
            xlAxisCategory = DirectCast(.Axes(, Excel.XlAxisGroup.xlSecondary), Excel.Axes)
            xlAxisCategory.Item(Excel.XlAxisType.xlValue).HasTitle = True
        ...
        oXl.ActiveChart.ChartTitle.Font.Size = 14
        '********************************************************************************************
