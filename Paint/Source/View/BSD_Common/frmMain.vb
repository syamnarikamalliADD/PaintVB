...
#Region " Declares "
    ...
    'NVSP 03/12/2017 - To hold the GUI predeetct and GUI post detects
    Friend msQueueTemp3 As String() = Nothing
    Friend msQueueTemp4 As String() = Nothing
    Friend msQueueTemp5 As String() = Nothing
    'NVSP 02/28/2017 - To hold all the predetect queue values
    Friend msPreDetectQueueData As String() = Nothing
    Friend msPreDetectTemp1 As String() = Nothing
    Friend msPreDetectTemp2 As String() = Nothing
    Friend msPreDetectTemp3 As String() = Nothing
    Friend msPreDetectTemp4 As String() = Nothing
    Friend msPreDetectTemp5 As String() = Nothing
    Friend msPreDetectTemp6 As String() = Nothing
    Friend msPreDetectTemp7 As String() = Nothing
    Friend msPreDetectTemp8 As String() = Nothing
    Friend msPreDetectTemp9 As String() = Nothing
    Friend msPreDetectTemp10 As String() = Nothing
    Friend msPreDetectTemp11 As String() = Nothing
    Friend msPreDetectTemp12 As String() = Nothing
    Friend msPreDetectTemp13 As String() = Nothing
    Friend msPreDetectTemp14 As String() = Nothing
    Friend msPreDetectTemp15 As String() = Nothing
    Friend msPreDetectTemp16 As String() = Nothing
    Friend msPreDetectTemp17 As String() = Nothing
    Friend msPreDetectTemp18 As String() = Nothing
    Friend msPreDetectTemp19 As String() = Nothing
    Friend msPreDetectTemp20 As String() = Nothing
    ...
    Friend Property ScatteredAccessEnableController(ByVal Index As Integer) As Boolean
        ...
        Get
            ...
            For Each oArm As clsArm In colControllers(Index).Arms
                If mnSAEnableArm(oArm.ArmNumber - 1) Then 'NRU 160922 added -1, most of the code was 0 (index) based.
                    Return True
                End If
            Next
            Return False
        ...
        Set(ByVal value As Boolean)
            ...
            For Each oArm As clsArm In colControllers(Index).Arms
                mnSAEnableArm(oArm.ArmNumber - 1) = value 'NRU 160922 added -1, most of the code was 0 (index) based.
            Next
        End Set
    End Property
#End Region
...
#Region " Routines "
    ...
    Private Sub subChangeZone()
        ...
        Try
            ...
            'NVSP 20161027 Change to the four different zones

            Select colZones.ActiveZone.LineDirection.ToUpper
                Case "LEFT"
                    mUctlQueue = New uctlQueueScreenLeft

                    Select Case colZones.ActiveZone.Name

                        Case "Prime"
                            mUctlBooth = New uctlBSDPrime
                            btnQueue.Visible = False
                        Case "Base"
                            mUctlBooth = New uctlBSDBase
                            btnQueue.Visible = True
                        Case "Mica"
                            mUctlBooth = New uctlBSDMica
                            btnQueue.Visible = False
                        Case "Clear"
                            mUctlBooth = New uctlBSDClear
                            btnQueue.Visible = False
                    End Select
            End Select
            ...
            'NRU 160928 Clear out "current screen" stuff so the new hotlinks don't try to send data to destroyed controls.
            'Tried setting them to the current values, but this turns your queue green.  (Hotlinks return data to controls before
            'they are ready and strangeness ensues.)
            gBSDFormCurrentScreen = Nothing
            gUctlCurrentScreen = Nothing
    ...
    Private Sub subStartHotlinks()
        ...
        Try
            ...
            Else
                ...
                'NRU 161007 Hotlinks still weren't being destroyed, this does the trick.
                mPLC.Dispose()
            ...
            With mPLC
                ...
                DoEvents()
                'NVSP 02/28/2017 - To hold all the GUI predetect and post detect queue values

                .TagName = sPrefix & "BoothQueue3"
                msQueueTemp3 = .PLCData

                DoEvents()

                .TagName = sPrefix & "BoothQueue4"
                msQueueTemp4 = .PLCData

                DoEvents()

                .TagName = sPrefix & "BoothQueue5"
                msQueueTemp5 = .PLCData

                DoEvents()
                'NVSP 02/28/2017 - To hold all the predetect queue values
                .TagName = sPrefix & "PreDetectQueue1"
                msPreDetectTemp1 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue2"
                msPreDetectTemp2 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue3"
                msPreDetectTemp3 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue4"
                msPreDetectTemp4 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue5"
                msPreDetectTemp5 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue6"
                msPreDetectTemp6 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue7"
                msPreDetectTemp7 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue8"
                msPreDetectTemp8 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue9"
                msPreDetectTemp9 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue10"
                msPreDetectTemp10 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue11"
                msPreDetectTemp11 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue12"
                msPreDetectTemp12 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue13"
                msPreDetectTemp13 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue14"
                msPreDetectTemp14 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue15"
                msPreDetectTemp15 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue16"
                msPreDetectTemp16 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue17"
                msPreDetectTemp17 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue18"
                msPreDetectTemp18 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue19"
                msPreDetectTemp19 = .PLCData
                DoEvents()
                .TagName = sPrefix & "PreDetectQueue20"
                msPreDetectTemp20 = .PLCData
...
Status, MessageBoxButtons.OK)
    ...
    Private Sub subInitializeForm()
        ...
        Try
            ...
            subInitFormText()
            'NVSP 02/28/2017 Added to handle carriors out of prime booth and before the start of base part detetct
    ...
    Private Sub subSizeUserControl(ByVal uControl As UserControl)
        ...
        End If
        'If (colZones.ActiveZone.ZoneNumber = 2) Then
        If Not (msPreDetectQueueData Is Nothing) Then
            gBSDFormCurrentScreen.LinkIndex = ePLCLink.myQueue
            gBSDFormCurrentScreen.PLCData = msPreDetectQueueData
            'End If
        End If
    End Sub
    ...
    Friend Function GetCachedData(ByVal LinkIndex As ePLCLink) As String()
        ...
        Select Case LinkIndex
            ...
            Case ePLCLink.Queue
                ...
                msQueueData = CType(MergeArrays(msQueueData, msQueueTemp3), String())
                msQueueData = CType(MergeArrays(msQueueData, msQueueTemp4), String())
                msQueueData = CType(MergeArrays(msQueueData, msQueueTemp5), String())
                Return msQueueData
            Case ePLCLink.myQueue
                msPreDetectQueueData = CType(MergeArrays(msPreDetectTemp1, msPreDetectTemp2), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp3), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp4), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp5), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp6), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp7), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp8), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp9), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp10), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp11), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp12), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp13), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp14), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp15), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp16), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp17), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp18), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp19), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp20), String())
...
#Region " Events "
    ...
    Private Sub mPLC_NewData(ByVal ZoneName As String, ByVal TagName As String, _
        ...
        Select Case TagName
            ...
            Case sPrefix & "RobotData", sPrefix & "RobotData2"
                ...
                gBSDFormCurrentScreen.PLCData = msRobotData
                'NVSP 02/28/2017 - To hold all the predetect queue values
            Case sPrefix & "BoothQueue1", sPrefix & "BoothQueue2", sPrefix & "BoothQueue3", sPrefix & "BoothQueue4", sPrefix & "BoothQueue5"
                If TagName = sPrefix & "BoothQueue1" Then
                    msQueueTemp1 = Data
                ElseIf TagName = sPrefix & "BoothQueue2" Then
                    msQueueTemp2 = Data
                ElseIf TagName = sPrefix & "BoothQueue3" Then
                    msQueueTemp3 = Data
                ElseIf TagName = sPrefix & "BoothQueue4" Then
                    msQueueTemp4 = Data
                Else
                    msQueueTemp5 = Data
                End If
            ...
            Case sPrefix & "PreDetectQueue1", sPrefix & "PreDetectQueue2", sPrefix & "PreDetectQueue3", sPrefix & "PreDetectQueue4", sPrefix & "PreDetectQueue5", _
                sPrefix & "PreDetectQueue6", sPrefix & "PreDetectQueue7", sPrefix & "PreDetectQueue8", sPrefix & "PreDetectQueue9", sPrefix & "PreDetectQueue10", _
                sPrefix & "PreDetectQueue11", sPrefix & "PreDetectQueue12", sPrefix & "PreDetectQueue13", sPrefix & "PreDetectQueue14", sPrefix & "PreDetectQueue15", _
                sPrefix & "PreDetectQueue16", sPrefix & "PreDetectQueue17", sPrefix & "PreDetectQueue18", sPrefix & "PreDetectQueue19", sPrefix & "PreDetectQueue20"
                If TagName = sPrefix & "PreDetectQueue1" Then
                    msPreDetectTemp1 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue2" Then
                    msPreDetectTemp2 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue3" Then
                    msPreDetectTemp3 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue4" Then
                    msPreDetectTemp4 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue5" Then
                    msPreDetectTemp5 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue6" Then
                    msPreDetectTemp6 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue7" Then
                    msPreDetectTemp7 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue8" Then
                    msPreDetectTemp8 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue9" Then
                    msPreDetectTemp9 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue10" Then
                    msPreDetectTemp10 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue11" Then
                    msPreDetectTemp11 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue12" Then
                    msPreDetectTemp12 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue13" Then
                    msPreDetectTemp13 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue14" Then
                    msPreDetectTemp14 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue15" Then
                    msPreDetectTemp15 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue16" Then
                    msPreDetectTemp16 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue17" Then
                    msPreDetectTemp17 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue18" Then
                    msPreDetectTemp18 = Data
                ElseIf TagName = sPrefix & "PreDetectQueue19" Then
                    msPreDetectTemp19 = Data
                Else
                    msPreDetectTemp20 = Data
                End If
                'NVSP 02/28/2017 - To hold all the predetect queue values
                msPreDetectQueueData = CType(MergeArrays(msPreDetectTemp1, msPreDetectTemp2), String())
                msPreDetectQueueData = CType(MergeArrays(msPreDetectQueueData, msPreDetectTemp3), String())
    ...
    Private Sub tmrSA_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tmrSA.Tick
        ...
        If gBSDFormCurrentScreen Is Nothing Then Exit Sub
        If meCurrentScreen = eBSDScreenSelect.Process Then
