...
#Region " Declares "
    ...
    Private Const mnSILHOUETTES As Integer = 0
    ...
    Private Const mnNumEstops As Integer = 4
    Private Const mnNumLimits As Integer = 3
    Private Const mnNumMaintLocks As Integer = 8
    'NVSP 03/12/2017 Predetect for Mica takes 15 positions in boothqueue and post detect starts at 16 instead of 11
    Private Const mnPOST_DETECT_Q1_POS_IN_DATA As Integer = 16
...
#Region " Routines "
    ...
    Friend Sub Initialize(Optional ByVal sParam As String = "") Implements BSDForm.Initialize
        ...
        'NVSP 03122017 GUI Carrior Graphic
        mBSDCommon.TrackingScale = -0.12
        mBSDCommon.TrackingStartPos = 1070 'NRU 160926
        mBSDCommon.TrackingQueueCountsBeforeShift = 300
        'BTK 01/13/10 added to flag which booth.
        'If frmMain.Line2 Then
        '    gpbFortressRight.Visible = False
        '    gpbFortressLeft.Visible = False
        'End If
        Call subSetToolTips()
    ...
    Private Sub subDoManDoors(ByVal e As System.Windows.Forms.PaintEventArgs)
        ...
        For i As Integer = nStart To nStart + (mnDOORS - 1)
            ...
            If (mnManDoorState And gnBitVal(i)) = gnBitVal(i) Then
                'door closed
                e.Graphics.DrawLine(oGreen, tDoors(nDoor).Hinge, tDoors(nDoor).Close)
            Else
                'door open
                e.Graphics.DrawLine(oRed, tDoors(nDoor).Hinge, tDoors(nDoor).Open)
            End If
        Next
    ...
    Private Sub subDoSilhouettes(ByVal e As System.Windows.Forms.PaintEventArgs)
        ...
        For i As Integer = nStart To nStart + (mnSILHOUETTES - 1)
            ...
            If (mnManDoorState And gnBitVal(i)) = gnBitVal(i) Then
                'Silhouette OK
                e.Graphics.DrawLine(oGreen, tSilhouettes(nSilhouette).Hinge, tSilhouettes(nSilhouette).Close)
            Else
                'Silhouette displaced
                e.Graphics.DrawLine(oRed, tSilhouettes(nSilhouette).Hinge, tSilhouettes(nSilhouette).Open)
            End If
        Next
    ...
    Private Sub subDoIntrusionLines(ByVal e As System.Windows.Forms.PaintEventArgs)
        ...
        'Hack to skip entrance intrusion
        For nLine As Integer = tIntLines.GetLowerBound(0) To tIntLines.GetUpperBound(0)
            'Dim nLine As Integer = 1
            If oPenPEC(nLine) Is Nothing Then Exit Sub
            e.Graphics.DrawLine(oPenPEC(nLine), tIntLines(nLine).StartPoint, tIntLines(nLine).EndPoint)
        Next
    ...
    Private Sub subSetUpManDoors()
        ...
        For i As Integer = 1 To mnDOORS
            ...
            ' MAS 20160908 Add in the two other doors
            With oDoorLbl
                Select Case i
                    Case 1
                        'Hinge Upper Left
                        ...
                        'Close Lower Left
                        tDoors(i).Close = New Point(.Left + 2, .Bottom - 2)
                        'Open Lower Right
                        tDoors(i).Open = New Point(.Right - 2, .Bottom - 2)
                    Case 2
                        'Hinge Upper Right
                        tDoors(i).Hinge = New Point(.Right - 2, .Top + 2)
                        'Close Lower Right
                        tDoors(i).Close = New Point(.Right - 2, .Bottom - 2)
                        'Open Lower Left
                        tDoors(i).Open = New Point(.Left + 2, .Bottom - 2)
                    Case 3
                        'Hinge Upper Left
                        tDoors(i).Hinge = New Point(.Left + 2, .Top + 2)
                        'Close Upper Right
                    ...
                    Case 4
                        ...
                        tDoors(i).Hinge = New Point(.Right - 2, .Top + 2)
                        'Close Upper Left
                        tDoors(i).Close = New Point(.Left + 2, .Top + 2)
                        'Open Lower Left
                        tDoors(i).Open = New Point(.Left + 2, .Bottom - 2)
                End Select
            End With 'oDoorLbl
        ...
        For i As Integer = 1 To mnSILHOUETTES
            ...
            With oGateLbl
                Select Case i
                    Case 1, 3
                        'Hinge Lower Right
                        tSilhouettes(i).Hinge = New Point(.Right - 2, .Bottom - 2)
                        'Close Upper Right
                        tSilhouettes(i).Close = New Point(.Right - 2, .Top + 2)
                        'Open Upper Left
                        tSilhouettes(i).Open = New Point(.Left + 2, .Top + 2)
                    Case 2, 4
                        'Hinge Upper Right 
                        tSilhouettes(i).Hinge = New Point(.Right - 2, .Top + 2)
                        'Close Lower Right
                        tSilhouettes(i).Close = New Point(.Right - 2, .Bottom - 2)
                        'Open Lower Left
                        tSilhouettes(i).Open = New Point(.Left + 2, .Bottom - 2)
                End Select
            End With 'oGateLbl
        Next 'i
    ...
    Private Sub subSetToolTips()
        ...
        With ttDesc
            ...
            .SetToolTip(picEstop01, gpsRM.GetString("psESTOP_FS_Ent"))
            .SetToolTip(picEstop022, gpsRM.GetString("psESTOP_FS_Exit"))
            ...
            .SetToolTip(picLimitSw0, gpsRM.GetString("psDETECT"))
        End With
    ...
    Private Sub subInitFormText()
        ...
        ' Date      By      Reason
        '********************************************************************************************
        'NVSP 6 -> 13 as there are 13 predetects
        Call InitCommonText(Me, 13, 0, mnuRobotContext, 6, mnuOnOff)

    End Sub

    Public Sub InitPLCData() Implements BSDForm.InitPLCData
        '********************************************************************************************
        'Description: Base out saved copy of PLC data so it'll process it all next
        '               time data is sent
        'Parameters: none
        'Returns:    none
        '
            ...
            Case ePLCLink.Robot
                ...
                For i As Integer = 0 To nUB
                    ...
                    If nOffSet = (eRobot.WordsPerRobot - 1) Then
                        ...
                        nRobNum += 1
                        bChangeFound = False
                        'i += 15             'NVSP 10/28/2016 15 words per equipment and 30 words for the robot controller in total
                    Else
                        nOffSet += 1
                    End If
...
#End Region
    ...
    Private Sub UctlExitInt_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UctlExitInt.Load

    End Sub

    Private Sub gpbHiddenStuff_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles gpbHiddenStuff.Enter
